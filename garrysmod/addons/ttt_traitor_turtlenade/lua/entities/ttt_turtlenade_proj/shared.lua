   ...
   if SERVER then
      ...
      self:SetDetonateExact(0)	
	     ...
	     for i=1,6 do
			...
			local headturtle = SpawnNPC(self:GetThrower(),spos, "npc_headcrab")
...
function TurtleNadeDamage(victim, dmg)
	...
	if attacker:IsValid() and attacker:IsNPC() and attacker:GetClass() == "npc_headcrab" and victim:IsTraitor() == false then
	...
	if victim:GetClass() == "npc_headcrab" then
		...
		if dmg:GetInflictor():GetClass() == "weapon_zm_improvised" then
			...
			victim:SetColor(Color(255,255,255,1))
